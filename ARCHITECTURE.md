...
Proposer
  │
  ├─► ProposalManager.commitProposal()     [1] Lock hash
  │
  ├─► ProposalManager.revealProposal()     [2] Reveal params → state: QUEUED
  │
Signers
  │
  ├─► AuthorizationLayer.authorizeProposal()  [3] M-of-N approval
  │
Executor
  │
  ├─► AresTreasury.queueExecution()           [4] Verify state + auth → TimelockEngine.queue()
  │
  │   (time passes)
  │
  ├─► AresTreasury.execute()                  [5] TimelockEngine.markExecuted()
  │                                                ProposalManager.markExecuted()
  │                                                target.call(calldata)   ← LAST
```
